...
users = Namespace('Authentication', description='Users authentication operations', path='/')

login_request_body_model = api.model('Login data model', {
    'email': fields.String(required=True, description='User\'s email', example='admin@gmail.com'),
    'psw': fields.String(required=True, description='User\'s password', example='123456')
})
register_request_body_model = api.model('Registration data model', {
    'nickname': fields.String(required=True, description='User\'s nickname', example='newuser'),
    'email': fields.String(required=True, description='User\'s email', example='newuser@gmail.com'),
    'psw': fields.String(required=True, description='User\'s password', example='123456')
})

users_model = api.model('Users model', {
    'id': fields.Integer,
...
class Login(Resource):
    ...
    @users.response(401, 'Bad Authentication data')
...
class Registration(Resource):
    ...
    def post(self):
        ...
        user_id = db.session.execute(insert(Users).values(request.json)).inserted_primary_key[0]
