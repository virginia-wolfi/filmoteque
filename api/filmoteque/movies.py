...
add_request_body_model = api.model(
    "Add_movie_data_model",
    {
        ...
        "year": fields.Integer(
            required=True, description="Movie's release year", example=1997
        ),
...
edit_request_body_model = api.model(
    "Edit_movie_data_model",
    {
        "title": fields.String(
            description="Movies' title", example="Full Metal Jacket"
        ),
        "year": fields.Integer(description="Movies' release year", example=1987),
...
parser.add_argument(
    "filter_by_title", type=str, location="values", help="maintains a partial match"
)
parser.add_argument(
    "filter_by_director", type=str, location="values", help="maintains a partial match"
)
...
@movies.route("/adding_movie", doc={"description": "Adds movie to the filmoteque"})
class AddMovie(Resource):
    ...
    def post(self):
        ...
        if to_insert["title"]:
            is_in_database = Movies.query.filter(
                Movies.title.ilike(to_insert["title"])
            ).first()
            if is_in_database:
                abort(
                    HTTP_400_BAD_REQUEST, "This movie title is already in the database"
                )
        director = to_insert.get("director")
        if len(director) < 4:
            return abort(
                HTTP_400_BAD_REQUEST, "Director's name must be 4 or more letters"
            )
        director = add_director(director)
        to_insert.pop("director")
        if not str(to_insert["year"]).isdigit() or not (
            1900 < int(to_insert["year"]) < 2025
        ):
            return abort(
                HTTP_400_BAD_REQUEST, "Check if the movie's release date is correct"
            )
        ...
        try:
            movie_id = db.session.execute(
                insert(Movies).values(to_insert)
...
@movies.route(
    "/<int:movie_id>/poster", doc={"description": "Movie poster related operations"}
)
...
class Poster(Resource):
    ...
    def post(self, movie_id):
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                db.session.commit()
                extra.info(
                    f"User added poster to movie {movie}", extra={"user": current_user}
                )
                return make_response(
                    jsonify({"message": "Movie poster added", "movie": str(movie)}),
                    HTTP_201_CREATED,
                )
            except:
                db.session.rollback()
    ...
    def delete(self, movie_id):
        ...
        db.session.commit()
        extra.info(
            f"User deleted poster from movie {movie}", extra={"user": current_user}
        )
        return {}, HTTP_204_NO_CONTENT


@movies.route(
    "/<int:movie_id>/", doc={"description": "Added movies related operations"}
)
@movies.param("movie_id", "Specify the id associated with the movie")
    ...
    def patch(self, movie_id):
        ...
        if request.json:
            if "title" in request.json:
                ...
                is_in_database = Movies.query.filter(
                    Movies.title.ilike(request.json["title"])
                ).first()
                if is_in_database and is_in_database != movie:
                    abort(
                        HTTP_400_BAD_REQUEST,
                        "This movie title is already in the database",
                    )
            if "director" in request.json:
                ...
                if len(director) < 4:
                    return abort(
                        HTTP_400_BAD_REQUEST,
                        "Director's name must be 4 or more letters",
                    )
                director = add_director(director)
                request.json["director_id"] = director.id
                request.json.pop("director")
            if "year" in request.json:
                if not str(request.json["year"]).isdigit() or not (
                    1900 < int(request.json["year"]) < 2025
                ):
                    return abort(
                        HTTP_400_BAD_REQUEST,
                        "Check if the movie's release year is correct",
                    )
        ...
        try:
            db.session.commit()
            extra.info(
                f"User edited movie {movie}: {request.json if request.json else ''},"
    ...
    def get(self):
        ...
        if filter_params or year:
            ...
            if director:
                sub_director = (
                    select(Movies.id)
                    .join(Directors)
                    .where(Directors.name.icontains(director))
                )
            ...
            if genres:
                sub_genres = (
                    ...
                    .join(movies_genres)
                    .join(Genres)
                    .where(Genres.name.in_(genres))
                )
                search_params.append(sub_genres)
            if year:
                sub_year = select(Movies.id).where(
                    Movies.year.between(lower_year, higher_year)
                )
        ...
        for f in result:
            output[str(f)] = {
                "title": f.title,
                "year": f.year,
                "director": f.director.name if f.director else "Null",
                "rate": f.rate,
