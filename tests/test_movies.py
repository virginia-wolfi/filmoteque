...
def test_adding_movie_unathorized(
    client,
    movie,
    title,
    director,
...
def test_uploading_poster(
    app, client, auth, movie, add_movie, file_part, file_name, status_code, message
):
...
def test_uploading_poster_user_unathorized(
    client, auth, add_movie, movie, user, status_code, message
):
    movie_id = add_movie
    auth.login(*user)
    with client:
...
def test_edit_movie(
    client, auth, movie, add_movie, user, query_string, json, status_code, message
):
    movie_id = add_movie
    auth.login(*user)
    with client:
...
@pytest.mark.parametrize(
    ...
    (
        (
            {"genre_1": "western", "filter_by_director": "tara"},
            200,
            b"Quentin Tarantino",
            1,
        ),
        ...
        (
            {"filter_by_year_after": 2026},
            400,
            b"Check if the movie's release year is correct",
            1,
        ),
        (
            {"filter_by_year_before": 1899},
            400,
            b"Check if the movie's release year is correct",
            1,
        ),
...
def test_search_movie_orderby(movie, insert_data):
    ...
    response = movie.search_movies(
        {"rate": "ASC", "year": "ASC", "filter_by_director": "Stanley"}
    )
    assert response.status_code == 200
